    ...
    def patch(self, ts_interval: tuple = None):
        ...
        def unet_wrapper(model_func, params):
            ...
            if pos_lats.shape[0] == 0:
                pos_lats = pos_lats.view(0, 1, 1, 1).expand(0, *input.shape[1:])
            if neg_lats.shape[0] == 0:
                neg_lats = neg_lats.view(0, 1, 1, 1).expand(0, *input.shape[1:])
            ...
            pos_shape_mismatch = pos_lats.shape[1:] != input.shape[1:]
            neg_shape_mismatch = neg_lats.shape[1:] != input.shape[1:]
            if pos_shape_mismatch or neg_shape_mismatch:
                warnings.warn(
